...
//#endif
   ...
   auto inside_coef = [=](const Vector &x) -> double
   {
      ...
      double epi = 0;
      rho *= (1.0 + epi * fabs(cos(theta)));
      //rho *= (1.0 + epi * cos(theta));
...
//#endif
   ...
   if (adaptive)
   {
      ...
      mesh_filename << "refined_mesh_0" << ".mesh";
      std::ofstream mesh_ofs_i(mesh_filename.str());
      mesh.Print(mesh_ofs_i);
      mesh_ofs_i.close();
      std::cout << "Mesh saved to: " << mesh_filename.str() << std::endl;
      ...
      for (int refinement_step = 1; refinement_step <= max_refinements; refinement_step++)
      {
         ...
         std::ostringstream mesh_filename;
         mesh_filename << "refined_mesh_" << refinement_step << ".mesh";
         std::ofstream mesh_ofs_i(mesh_filename.str());
         mesh.Print(mesh_ofs_i);
         mesh_ofs_i.close();
