...
int main(int argc, char *argv[])
{
    ...
    ofstream fout("solver_"+to_string(solver_type)+"_alpha.dat");
fout << "#alpha dt N_calls L2_error\n";

vector<real_t> alpha_values = {0.1, 0.3, 0.5, 1.0, 2.0, 5.0}; // Example range of alpha

double target_error = 1e-5;

for (real_t alpha : alpha_values)
{
    cout<<alpha<<endl;
    oper = CountingRelaxOp(tau, alpha, d0); // Reinitialize operator for each alpha
    ode_solver->Init(oper);

    real_t dt_low = 1e-6, dt_high = 5.0, dt = 1.0;
    real_t actual_error;
    ...
    if (fabs(actual_error - target_error) / target_error > 0.05){
        cout<<"Maximum iteration number achieved for alpha="<<alpha<<endl;
    } else {
        fout << alpha << " " << dt << " " << oper.GetCounter() << " " << actual_error << "\n";
    }
}
