...
int main()
{
    ...
    const vector<int> solvers = {2};
    vector<real_t> omega_t;
    for(real_t ot=1e-3; ot<=1e1; ot*=sqrt(sqrt(10))) omega_t.push_back(ot);
    ...
    for(int st: solvers)
    {
        ...
        ofstream fout("solver_"+to_string(st)+"_frequency_1e8.dat");
        fout<<"#omega_tau  dt  calls  L2err\n";
        for(real_t ot: omega_t)
        {
            real_t tau=1.0, omega=ot/tau;
            cout<<"omega = "<<omega<<":"<<endl;
            CountingRelaxOp op(tau,omega,d0);
            ode->Init(op);

            real_t lo=1e-4, hi=1.0, dt=0.1, err=0;
            int calls=0;
            bool max_iter = false;
