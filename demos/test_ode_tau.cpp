...
int main()
{
    ...
    Vector d0(sz), m0(sz), m(sz);
    {
        ...
        VectorFunctionCoefficient d_coef(5,
          [](const Vector &x, Vector &v){
            real_t s1 = sin(2*M_PI*x[0]),
                   c2 = cos(2*M_PI*x[1]),
                   s3 = sin(2*M_PI*x[2]);
            for (int j=0; j<5; j++) v(j) = (j+1)*s1*c2*s3;
          });
        d_gf.ProjectCoefficient(d_coef);
        d_gf.GetTrueDofs(d0);

        VectorFunctionCoefficient m_coef(5,
          [](const Vector &x, Vector &v){
            real_t c1 = cos(M_PI*x[0]),
                   s2 = sin(M_PI*x[1]),
                   c3 = cos(M_PI*x[2]);
            for (int j=0; j<5; j++) v(j) = (j+1)*c1*s2*c3;
          });
        m_gf.ProjectCoefficient(m_coef);
        m_gf.GetTrueDofs(m0);

        m = m0;
    }

    const vector<int> solvers = {1, 2, 3, 4, 21, 99};  // FE, RK2, RK3, RK4, BE, Bailey
    vector<real_t> taus;
    for (real_t T = 1e-2; T <= 1e2; T *= sqrt(10.0))
        taus.push_back(T);

    const real_t eps = 1e-3, omega = 1.0, Tfinal = 1000.0;
    ...
    for (int st : solvers)
    {
        ...
        ofstream fout("solver_" + to_string(st) + "_stiffness_eps1e8.dat");
        fout << "#tau  dt  calls  L2err\n";
        ...
        for (real_t tau : taus)
        {
            ...
            if (st == 99)
            {
                static_cast<BaileySolver_test*>(ode.get())
                    ->SetTau(tau);
            }
            ...
            real_t lo = 1e-4, hi = 500.0, dt = dt_old, err = 0;
            int calls = 0;
            int max_iter = false;
            for (int it=0; it<25; it++)
            {
                ...
                ma -= m; err = ma.Norml2()/m.Norml2(); calls = op.GetCounter();
                if (st == 99) cout<<"l2 error: "<<err<<endl;
                if (fabs(err - eps)/eps < 0.05) break;
            ...
            if (!max_iter)
                fout << tau << "  " << dt << "  " << calls << "  " << fabs(err - eps)/eps << "\n";
