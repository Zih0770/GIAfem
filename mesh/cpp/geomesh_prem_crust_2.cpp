...
static void createConcentricSphericalLayers(
    ...
    std::vector<int> &outerTwoShellsOut 
) {
    premVolumeTagsOut.clear();
    outerTwoShellsOut.clear(); 
    ...
    for (int i = 1; i < numLayers; ++i) {
        ...
        if(!isLastTwo) {
            ...
            for (int v : vols) outerTwoShellsOut.push_back(v); 
        }
    }
                                 ...
                                 double &rMoMin, double &rMoMax, double &rTpMin, double &rTpMax)
{
    ...
    for (int j = 0; j < nLat; ++j) {
        double lat = -90.0 + 180.0 * ((j + 0.5) / (double)nLat);        
