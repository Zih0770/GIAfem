...
extractLayerBoundaries(const std::string &fileName, double Rref, double &R_out)
{
    ...
    while (std::getline(file, line)) {
        ...
        if (!std::isnan(prevR) && std::abs(r - prevR) < 1e-6 && layerCount < 2) {
...
static void computeMohoTopRadii(const DiscreteTriMesh &seed,
                                ...
                                double &rMohoRep, double &rTopRep, double &vis_fac)
{
    ...
    for (int a=0; a<nn; ++a) {
        ...
        double rM = 1.0 + d2 * vis_fac;            // Moho radius
        double rT = 1.0 + (d1 + d2) * vis_fac;     // Topography radius
...
int main(int argc, char **argv) {
    ...
    std::string outputFileName = "mesh/prem_with_crust_vis";
    ...
    for (int i=1; i<argc; ++i) {
        ...
        else if (arg == "-vis_fac" && i+1<argc) vis_fac = std::stod(argv[++i]);
    }
    ...
    if (haveCrust) {
        ...
        computeMohoTopRadii(seed, G, Rmeter, rM, rT, rMohoRep, rTopRep, vis_fac);
